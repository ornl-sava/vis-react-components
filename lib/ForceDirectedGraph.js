...
var ForceDirectedGraph = function (_React$Component) {
  ...
  _createClass(ForceDirectedGraph, [{
    ...
    value: function setSim(props) {
      ...
      this.simulation.force('link', d3.forceLink().id(function (d, i) {
        return i;
      })).force('charge', d3.forceManyBody().strength(-5)).force('center', d3.forceCenter(props.chartWidth / 2, props.chartHeight / 2));
    ...
    value: function draw(props) {
      ...
      this.state.nodes.map(function (d, i) {
        var circleProps = {
          'data-id': i,
          'r': props.radius,
          'cx': Math.random() * _this3.xScale.bandwidth() + _this3.xScale(d.hour),
          'cy': Math.random() * props.chartHeight,
          'fill': _this3.colScale(d.hour),
          'data-events': d.events
          // 'onMouseEnter': this.onEnter,
          // 'onMouseLeave': this.onLeave,
          // 'onClick': this.onClick
        };
        nodeList.push(_react2.default.createElement('circle', _extends({ key: 'cir-id' + i + '-hr-' + d.hour }, circleProps)));
      });
