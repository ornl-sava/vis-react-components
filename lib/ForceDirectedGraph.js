...
var ForceDirectedGraph = function (_React$Component) {
  ...
  function ForceDirectedGraph(props) {
    ...
    _this.nodes = JSON.parse(JSON.stringify(props.nodes));
    _this.links = JSON.parse(JSON.stringify(props.links));
  ...
  _createClass(ForceDirectedGraph, [{
    ...
    value: function onEnter(event) {
      ...
      var tooltipD = { label: type + event.target.getAttribute('data-id') + ' at Hour ' + target.hour, counts: target.events.length };
    ...
    value: function falseStart(props) {
      ...
      if (props.dataType === 'list') {
        ...
        this.links.map(function (d, i) {
          d.source = _this3.nodes[props.links[i].source];
          d.target = _this3.nodes[props.links[i].target];
        });
      } else {
