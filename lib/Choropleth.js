...
var Choropleth = function (_React$Component) {
  ...
  function Choropleth(props) {
    ...
    _this.state = {
      selectedColorScale: _d2.default.scale.quantile(),
      unselectedColorScale: _d2.default.scale.quantile()
    };

    _this.projection = _d2.default.geo.equirectangular();
    _this.path = _d2.default.geo.path().projection(_this.projection);
  ...
  _createClass(Choropleth, [{
    ...
    value: function renderMap() {
      ...
      var tempSelectedColorScale = _d2.default.scale.linear().domain([0, this.props.numColorCat]).range([this.props.selectedMinColor, this.props.selectedMaxColor]).interpolate(_d2.default.interpolateHcl);

      var tempUnselectedColorScale = _d2.default.scale.linear().domain([0, this.props.numColorCat]).range([this.props.unselectedMinColor, this.props.unselectedMaxColor]).interpolate(_d2.default.interpolateHcl);
      ...
      _d2.default.range(this.props.numColorCat).map(function (i) {
        selectedColorRange.push(tempSelectedColorScale(i));
        unselectedColorRange.push(tempUnselectedColorScale(i));
      });

      this.state.selectedColorScale.domain(colorDomain).range(selectedColorRange);

      this.state.unselectedColorScale.domain(colorDomain).range(unselectedColorRange);
      ...
      var getColor = function getColor(id) {
        ...
        if (typeof datum !== 'undefined') {
          color = datum[_this3.props.selectedField] === _this3.props.selectedValue ? _this3.state.selectedColorScale(datum[_this3.props.valueField]) : _this3.state.unselectedColorScale(datum[_this3.props.valueField]);
        }
        return color;
      };
