...
var Axis = function (_React$Component) {
  ...
  function Axis(props) {
    ...
    _this.state = { range: 0, ticks: 0, scaleType: scaleType };
    _this.axis = _d2.default.svg.axis().scale(props.scale).orient(props.orient).ticks(props.tickCount ? props.tickCount : 0);
    return _this;
  }
  ...
  _createClass(Axis, [{
    ...
    key: 'resizeAxis',
    value: function resizeAxis(props) {
      var _this2 = this;

      var thisNode = _reactDom2.default.findDOMNode(this);
      var parentNode = thisNode.parentNode;
      var selector = '.' + props.className.replace(/ /g, '.');
      var selection = _d2.default.select(parentNode).select(selector);
      var tickCount = 0;
      var tickValues = [];
      ...
      if (props.data.length > 0) {
        ...
        if (this.state.scaleType === 'ordinal') {
          (function () {
            ...
            if (props.orient === 'top' || props.orient === 'bottom') {
              var binWidth = Math.floor(props.scale.range()[tickCount - 1] / tickCount);
            ...
            _this2.axis.tickFormat(function (d) {
        ...
        if (props.tickFormat) {
          this.axis.tickFormat(function (d, i) {
            return props.tickFormat(d, i);
          });
        }
      }

      this.axis.scale(props.scale).orient(props.orient).tickValues(tickValues).ticks(tickCount);
