...
var TopicFlow = function (_React$Component) {
  ...
  function TopicFlow(props) {
    ...
    _this.topics = [];
    _this.barWidth = 0;
    _this.bins = _this.initTopics(props);
    return _this;
  }
  ...
  _createClass(TopicFlow, [{
    ...
    value: function updateDomain(props) {
      var xDomain = Object.keys(props.data);
      this.xScale.domain(xDomain);
      this.yScale.domain(d3.range(-1, props.maxTopics + 2, 1));
    }
  }, {
    key: 'updateRange',
    value: function updateRange(props) {
      this.xScale.range([0, props.chartHeight]);
    ...
    value: function initTopics(props) {
      ...
      var svgTopicBars = props.adjacencyList.map(function (dataArr, i) {
        if (dataArr.hour < props.numTData) {
          ...
          var posY = _this2.yScale(dataArr.topicID);
          var posX = _this2.xScale(dataArr.hour);
          var fontSize = 12;
    ...
    value: function moveTopics() {
      return _react2.default.createElement(
        'g',
        { className: 'topicFlow', transform: 'translate(' + this.state.moveX + ',' + 0 + ')' },
        this.topics
      );
    }
  }, {
