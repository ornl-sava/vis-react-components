...
var binByNumeric = function binByNumeric(data, accessor, range, numBins) {
  ...
  var _loop = function _loop(i) {
    bin = data.filter(function (d) {
      return d[accessor] < range[0] + (i + 1) * step && d[accessor] >= range[0] + i * step;
    });
...
var HybridScatterHeatmap = exports.HybridScatterHeatmap = function (_React$Component) {
  ...
  function HybridScatterHeatmap(props) {
    ...
    _this.state = {
      ...
      xScale: _d2.default.time.scale().nice(_d2.default.time.second, 0.5),
      scatterColorScale: _d2.default.scale.linear(),
      heatmapColorScale: _d2.default.scale.quantile(),
  ...
  _createClass(HybridScatterHeatmap, [{
    ...
    value: function createChart() {
      ...
      // Create svg
      var svg = _d2.default.select(root).append('svg');
      ...
      header.append('text').attr('class', 'label time').text('Displaying events from ' + (0, _moment2.default)(this.endTime).format('YYYY-MM-DD h:mm:ss') + ' to ' + (0, _moment2.default)(this.props.startTime).format('YYYY-MM-DD h:mm:ss'));
      ...
      this.state.scatterColorScale.domain(this.props.yDomain).range([this.props.minScatterColor, this.props.maxScatterColor]).interpolate(_d2.default.interpolateHcl);

      // Create color scale for heatmap
      var colors = [];
      var tempColorScale = _d2.default.scale.linear().domain([0, this.props.numColorCat]).range([this.props.minHeatmapColor, this.props.maxHeatmapColor]);

      _d2.default.range(this.props.numColorCat).map(function (d) {
    ...
    value: function updateScatter() {
      ...
      var root = this.refs.root;
      var chart = _d2.default.select(root).select('svg').select('.chart');
    ...
    value: function updateHeatmap() {
      ...
      // Enter Bins
      bins.enter().append('rect').attr('opacity', function (d, i) {
        return 1 - heatmap[d.rowIndex][i].active;
      }).attr('class', 'bin').on('click.heatmap.' + this.props.clsName, function (d, i) {
        // Need to have reference to dynamic scope for access to d3 element, so no es6
        self.props.heatmapOnClick(d, i);
        var bin = _d2.default.select(this);
      ...
      // Update Bins
      bins.transition().duration(100).attr('x', function (d) {
      ...
      columnMarkers.enter().append('rect').attr('class', 'markerCol').on('click.markerCol', function (d, i) {
        if (_d2.default.event.shiftKey) {
          ...
          for (var row = 0; row < _this4.props.heatmapVertDivisions; row++) {
            ...
            _d2.default.select(bins[row][i]).style('opacity', 1 - heatmap[row][i].active);
          }
          self.updateChart();
        } else {
      ...
      columnMarkers.transition().duration(100).attr('x', function (d, i) {
        return _this4.state.xScale(_this4.endTime + d.key);
      }).attr('y', function (d, i) {
    ...
    value: function updateAxes() {
      var root = this.refs.root;
      var svg = _d2.default.select(root).select('svg');
      var chart = svg.select('.chart');

      chart.select('.header .time').text('Displaying events from ' + (0, _moment2.default)(this.endTime).format('YYYY-MM-DD h:mm:ss') + ' to ' + (0, _moment2.default)(this.props.startTime).format('YYYY-MM-DD h:mm:ss'));

      chart.select('.x.axis').call(_d2.default.svg.axis().orient('bottom').scale(this.state.xScale).ticks(5).tickFormat(function (a) {
        var format = _d2.default.time.format('%I:%M:%S');
        return format(a);
      }));

      chart.select('.y.axis').call(_d2.default.svg.axis().orient('left').scale(this.state.yScale));
    }
  }, {
    key: 'resizeChart',
    value: function resizeChart() {
      ...
      chart.select('.header .reset').attr({
        'x': chartWidth,
        'y': -this.props.margin.top + 1,
        'dy': '0.71em'
      }).style({
      ...
      chart.select('.x.axis').attr('transform', 'translate(0,' + chartHeight + ')').call(_d2.default.svg.axis().orient('bottom').scale(this.state.xScale)).select('.label').attr('x', chartWidth).attr('y', -6);

      chart.select('.y.axis').call(_d2.default.svg.axis().orient('left').scale(this.state.yScale)).select('.label').attr('transform', 'rotate(-90)').attr('y', 6).attr('dy', '.35em');
