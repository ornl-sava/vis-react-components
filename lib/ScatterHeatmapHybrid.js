...
var binByNumeric = function binByNumeric(data, accessor, range, numBins) {
  ...
  var _loop = function _loop(i) {
    var bin = data.filter(function (d) {
...
var HybridScatterHeatmap = exports.HybridScatterHeatmap = function (_React$Component) {
  ...
  _createClass(HybridScatterHeatmap, [{
    ...
    value: function createChart() {
      ...
      header.append('text').attr('class', 'label time').text('Displaying events from ' + d3.timeFormat('%x %X')(new Date(this.endTime)) + ' to ' + d3.timeFormat('%x %X')(new Date(this.props.startTime)));
      ...
      this.state.scatterColorScale.domain(this.props.yDomain).range([this.props.minScatterColor, this.props.maxScatterColor]).interpolate(d3.interpolateHcl);

      // Create color scale for heatmap
      var colors = [];
      var tempColorScale = d3.scaleLinear().domain([0, this.props.numColorCat]).range([this.props.minHeatmapColor, this.props.maxHeatmapColor]);

      d3.range(this.props.numColorCat).map(function (d) {
    ...
    value: function updateAxes() {
      ...
      chart.select('.header .time').text('Displaying events from ' + d3.timeFormat('%x %X')(new Date(this.endTime)) + ' to ' + d3.timeFormat('%x %X')(new Date(this.props.startTime)));

      chart.select('.x.axis').call(d3.axisBottom().scale(this.state.xScale).ticks(5).tickFormat(function (a) {
        var format = d3.timeFormat('%I:%M:%S');
        return format(a);
      }));

      chart.select('.y.axis').call(d3.axisLeft().scale(this.state.yScale));
    }
  }, {
    key: 'resizeChart',
    value: function resizeChart() {
      ...
      chart.select('.x.axis').attr('transform', 'translate(0,' + chartHeight + ')').call(d3.axisBottom().scale(this.state.xScale)).select('.label').attr('x', chartWidth).attr('y', -6);

      chart.select('.y.axis').call(d3.axisLeft().scale(this.state.yScale)).select('.label').attr('transform', 'rotate(-90)').attr('y', 6).attr('dy', '.35em');
