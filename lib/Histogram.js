...
var Histogram = function (_React$Component) {
  ...
  _createClass(Histogram, [{
    ...
    value: function updateDomain(props, state) {
      ...
      var xDomain = props.data[0].bins.map(function (bin) {
    ...
    key: 'sortData',
    value: function sortData(props, state) {
      // NOTE: This WILL mutate the prop
      // Sort first bin and then sort rest of bins accordingly
      var sortArr = [];
      props.data[0].bins.sort(function (a, b) {
        var i = 0;
        if (state.sortBy === 'x') {
          i = state.sortOrder === 'Ascending' ? _d2.default.ascending(a.x, b.x) : _d2.default.descending(a.x, b.x);
        } else {
          var useBin = state.sortTypes.indexOf(props.data[0].type) > -1 || state.sortTypes.length === 0;
          var ya = useBin ? a.y : 0;
          var yb = useBin ? b.y : 0;
          for (var j = 1; j < props.data.length; j++) {
            var _useBin = state.sortTypes.indexOf(props.data[j].type) > -1 || state.sortTypes.length === 0;
            if (_useBin) {
              props.data[j].bins.forEach(function (d, i) {
                if (d.x === a.x) {
                  ya += d.y;
                }
                if (d.x === b.x) {
          ...
          i = state.sortOrder === 'Ascending' ? ya - yb : yb - ya;
        }
        sortArr.push(i);
        return i;
      });
      ...
      var _loop = function _loop(i) {
        var j = 0;
        props.data[i].bins.sort(function (a, b) {
          return sortArr[j++];
        });
      };
