...
var Heatmap = function (_React$Component) {
  ...
  function Heatmap(props) {
    ...
    _this.colorScale = d3.scaleQuantile();
      ...
      if (props.data.length > 0 && props.data[0].bins.length > 0) {
        ...
        if (xDomain.length === 0) {
          ...
          if (props.xScale.type === 'ordinalBand') {
            ...
            xDomain = d3.extent(props.data[0].bins, function (d) {
              return d[props.xAccessor.key];
            });
        ...
        if (yDomain.length === 0) {
          ...
          if (props.yScale.type === 'ordinalBand') {
            ...
            yDomain = [0.000001, d3.max(props.data, function (d) {
              return d[props.yAccessor.key];
            })];
        ...
        var yMax = props.colorPerRow ? d3.max(props.data, function (d, i) {
          return d3.max(d.bins, function (e, j) {
            return e[props.xAccessor.value];
          });
        }) : d3.max(props.data, function (d, i) {
          return d[props.yAccessor.value];
        });

        var tempColorScale = d3.scaleLinear().domain([0, yMax]).range([props.minColor, props.maxColor]).interpolate(d3.interpolateHcl);
