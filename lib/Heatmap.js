...
var Heatmap = function (_React$Component) {
  ...
  function Heatmap(props) {
    ...
    _this.state = {
      colorScale: _d2.default.scale.quantile(),
  ...
  _createClass(Heatmap, [{
    ...
    value: function componentWillReceiveProps(nextProps) {
      if (nextProps.data.length > 0 && nextProps.data[0].bins.length > 0) {
        ...
        if (xDomain.length === 0) {
          ...
          var offset = nextProps.data[0].bins[1][nextProps.xAccessor.key] - nextProps.data[0].bins[0][nextProps.xAccessor.key];
          if (nextProps.xScaleType === 'ordinalBand') {
            xDomain = nextProps.data[0].bins.map(function (d) {
              return d[nextProps.xAccessor.key];
            });
          } else {
            xDomain = _d2.default.extent(nextProps.data[0].bins, function (d) {
              return d[nextProps.xAccessor.key];
            });
        ...
        var yDomain = nextProps.yDomain;
        if (yDomain.length === 0) {
          // NOTE: Computing offset so proper xDomain is given for time scales
          // Nth bin has a start time of it's key; so it's 'end time'
          // must be taken into consideration
          var _offset = nextProps.data[0][nextProps.yAccessor.key] - nextProps.data[0][nextProps.yAccessor.key];
          if (nextProps.yScaleType === 'ordinalBand') {
            yDomain = nextProps.data.map(function (d) {
              return d[nextProps.yAccessor.key];
            });
          } else {
            yDomain = [0.000001, _d2.default.max(nextProps.data, function (d) {
              return d[nextProps.yAccessor.key];
            })];
            yDomain[1] = yDomain[1] + _offset;
          }
        }
        ...
        var yMax = nextProps.colorPerRow ? _d2.default.max(nextProps.data, function (d, i) {
          return _d2.default.max(d.bins, function (e, j) {
            return e[nextProps.xAccessor.value];
          });
        }) : _d2.default.max(nextProps.data, function (d, i) {
          return d[nextProps.yAccessor.value];
        });

        var tempColorScale = _d2.default.scale.linear().domain([0, yMax]).range([nextProps.minColor, nextProps.maxColor]).interpolate(_d2.default.interpolateHcl);

        var colorDomain = [0, 1];
        var colorRange = [nextProps.minColor];
        var colorDomainBand = yMax / (nextProps.numColorCat - 1);
        ...
        this.state.colorScale.domain(colorDomain).range(colorRange);
      }
    }
  }, {
    key: 'onClick',
    ...
    key: 'renderHeatmap',
    value: function renderHeatmap(props) {
      var _this2 = this;
        ...
        props.data.map(function (d, i) {
          ...
          return d.bins.map(function (e, j) {
            ...
            var rectProps = {
              ...
              'width': width,
              'height': height,
              'fill': _this2.state.colorScale(e[props.xAccessor.value]),
              'onMouseEnter': _this2.onEnter,
              'onMouseLeave': _this2.onLeave,
              'onClick': _this2.onClick
            };
            return _react2.default.createElement('rect', _extends({ key: i + j }, rectProps));
          });
        })
      );
    }
